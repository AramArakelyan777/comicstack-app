...
class StatusController {
    async addStatus(req, res, next) {
        try {
            ...
            `;
            const existingStatusResult = await pool.query(existingStatusQuery, [userId, comicId]);
            ...
            if (existingStatusResult.rows.length > 0) {
                ...
                const updateStatusQuery = `
                    ...
                    RETURNING *;
                `;
                const updatedStatusResult = await pool.query(updateStatusQuery, [status, userId, comicId]);
                ...
                const insertStatusQuery = `
                    ...
                    RETURNING *;
                `;
                const newStatusResult = await pool.query(insertStatusQuery, [userId, comicId, status]);
    ...
    async deleteStatus(req, res, next) {
        try {
            ...
            const deleteStatusQuery = `
                ...
                RETURNING *;
            `;
            const deleteStatusResult = await pool.query(deleteStatusQuery, [userId, comicId]);
